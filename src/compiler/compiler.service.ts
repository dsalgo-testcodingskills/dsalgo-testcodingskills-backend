...
export class CompilerService {
  ...
  if (
    question.outputType === '2d_array_int' ||
    question.outputType === '2d_array_char'
  ) {
    return `arr = ${functionCall}; console.log("<logsOutputSeprator>" + JSON.stringify(arr));`;
  } else if (
    question.outputType === 'array_int' ||
    question.outputType === 'array_char'
  ) {
...
async getConvertedOutput(userOutput, outputType, language) {
  try {
    ...
    if (userOutput && (outputType === 'array_char' || outputType === 'array_int')) {
      ...
      convertedOutput = outputType === 'array_int'
        ? convertedOutput.map(Number)
        : convertedOutput;
    } else if (userOutput && (outputType === 'int' || outputType === 'boolean')) {
      if (language === 'cpp' && outputType === 'boolean')
        userOutput = userOutput === '1' ? true : userOutput === '0' ? false : userOutput;
    ...
    } else if (userOutput && (outputType === '2d_array_int' || outputType === '2d_array_char')) {
      // ← ADD THIS BLOCK
      convertedOutput = JSON.parse(userOutput);
    } else {
    ...
    exec(executeCommand, { timeout: 10000 }, async (error, stdout, stderr) => {
      if (stdout) {
        ...
        const isMatch =
          question.outputType === '2d_array_int' ||
          question.outputType === '2d_array_char'
            ? JSON.stringify(sortMatrix(convertedOutput)) ===
              JSON.stringify(sortMatrix(question.testCases[testCaseIndex].output))
